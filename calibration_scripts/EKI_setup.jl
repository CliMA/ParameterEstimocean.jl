...
function loss_reduction(nll, nll_validation, initial_parameters, kwargs)
    params, losses, mean_vars = eki_unidimensional(nll, initial_parameters; kwargs...)
    # println(losses[end])
...
function plot_prior_variance_and_obs_noise_level(nll, nll_validation, initial_parameters, directory; vrange=0.40:0.025:0.90, nlrange=-2.5:0.1:0.5)
    ...
    for i in 1:length(Γθs)
        for j in 1:length(Γys)
            ...
            Γθ = Γθs[i]
            Γy = Γys[j]
            train_loss_reduction, val_loss_reduction = loss_reduction(nll, nll_validation, initial_parameters, (stds_within_bounds=Γθ, noise_level=Γy, N_iter=10, N_ens=10, informed_priors=false))
