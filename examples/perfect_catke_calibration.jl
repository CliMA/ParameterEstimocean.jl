...
weight_distances = [norm(θ_mean[:, iter] - θ★) for iter = 1:iterations]
output_distances = [norm(forward_map(calibration, θ̅(iter))[:, 1] - y) for iter = 1:eki.iteration]
ensemble_variances = [varθ(iter) for iter = 1:eki.iteration]
...
save("distributions_catke_eki.svg", f);
nothing; #hide
