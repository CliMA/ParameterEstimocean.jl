...
ensemble_simulation, θ★ = build_ensemble_simulation(observations; Nensemble=2)
...
fig = Figure()
ax_top = Axis(fig[1, 1], xlabel = "convective κᶻ [m² s⁻¹]")
density!(ax_top, convective_κz_samples)
xlims!(ax_top, 0, 20)

ax_bottom = Axis(fig[2, 1], xlabel = "background κᶻ [m² s⁻¹]")
...
θ★ = [θ★.convective_κz, θ★.background_κz]

G = forward_map(calibration, θ★)
y = observation_map(calibration)

# The `forward_map` output `G` is a two-dimensional matrix whose first dimension is the size of the state space
# and whose second dimension is the `ensemble_size`. In the case above, all columns of `x` are identical.

@show mean(G, dims=2) ≈ y
