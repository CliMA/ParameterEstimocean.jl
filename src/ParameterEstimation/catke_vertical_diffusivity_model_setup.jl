...
function DataSet(LESdata, p::Parameters{UnionAll}; 
    ...
    model = CATKEVerticalDiffusivityModel.EnsembleModel(td_batch; N_ens=ensemble_size, 
                                            parameter_specific_kwargs[p.RelevantParameters]...)

    loss = LossFunction(model, td_batch, Δt, p.ParametersToOptimize; data_weights=[1.0 for td in td_batch],
                                                           relative_weights)
