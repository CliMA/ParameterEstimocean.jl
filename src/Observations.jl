...
struct SyntheticObservations{F, R, G, T, P, M, Þ} <: AbstractObservation
...
function SyntheticObservations(path=nothing;
    ...
    if isnothing(field_time_serieses)
        raw_time_serieses = NamedTuple(name => FieldTimeSeries(path, string(name); times, architecture)
    ...
    if isnothing(regrid)
        ...
        for (field_name, ts) in zip(keys(raw_time_serieses), raw_time_serieses)
            ...
            new_ts = FieldTimeSeries{LX, LY, LZ}(grid, times; boundary_conditions, architecture)
...
function get_names(observations, name_getter)
    names = Set()
...
"""
observation_names(obs::Vector{<:SyntheticObservations}) = get_names(obs, observation_names)

"""
    forward_map_names(observations::Vector{<:SyntheticObservations})

Return a Set representing the union of all names in `obs`.
"""
forward_map_names(obs::Vector{<:SyntheticObservations}) = get_names(obs, forward_map_names)
