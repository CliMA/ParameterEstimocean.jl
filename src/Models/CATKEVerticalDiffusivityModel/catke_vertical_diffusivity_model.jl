...
"""
    EnsembleModel(observations::OneDimensionalTimeSeriesBatch; architecture = CPU(), N_ens = 50, kwargs...)
...
"""
function EnsembleModel(observations::OneDimensionalTimeSeriesBatch; architecture = CPU(), N_ens = 50, kwargs...)

    data_grid = observations[1].grid
    grid = ColumnEnsembleGrid(data_grid; size=(N_ens, length(observations), data_grid.Nz))

    closure = [CATKEVerticalDiffusivity(Float64; warning=false, kwargs...) for i=1:N_ens, j=1:length(observations)]

    coriolis = [FPlane(f=td.constants[:f]) for i=1:N_ens, td in observations]
    coriolis = arch_array(architecture, coriolis)

    bc_matrix(f) = [f(td.boundary_conditions) for i = 1:N_ens, td in observations]
